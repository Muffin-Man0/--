...
Paginator.prototype = {
    ...
    _renderTemplate : function (container, template, id_base, hide) {
        ...
        if (container.id.indexOf("paginator0") >= 0) {  // wc 陈伟修改 不显示头部分页
            $(container).remove();
        }
        //else if (container.id.indexOf("paginator1") >= 0) {
        //    $(container).html($(container).html() + '<input type="button" onclick="if (!(art == undefined || art == null)) {art.dialog.close();}" value="关闭" style="float:right;"/>');
        //}
    },
